...
var s = function s() {
  ...
  combineVals = function combineVals(row) {
    for (i = 3; i >= 1; i--) {
      var a = row[i];
      var b = row[i - 1];
  ...
  slide = function slide(row) {
    var arr = row.filter(function (val) {
      return val;
    });
    var missing = 4 - arr.length;
    var zeroes = Array(missing).fill(0);
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "51981" + '/');
